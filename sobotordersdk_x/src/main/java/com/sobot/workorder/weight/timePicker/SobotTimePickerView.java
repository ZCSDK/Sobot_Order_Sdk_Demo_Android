package com.sobot.workorder.weight.timePicker;
...
import com.sobot.workorder.weight.timePicker.listener.SobotCustomListener;
import com.sobot.workorder.weight.timePicker.view.SobotBasePickerView;
import com.sobot.workorder.weight.timePicker.view.SobotWheelTime;
...
public class SobotTimePickerView extends SobotBasePickerView implements View.OnClickListener {
    ...
    public static class Builder {
        private String layoutRes = "sobot_wo_pickerview_time";
        private SobotCustomListener customListener;
    ...
    private void initView(Context context) {
        ...
        if (customListener == null) {
            LayoutInflater.from(context).inflate(SobotResourceUtils.getIdByName(context, "layout", "sobot_wo_pickerview_time"), contentContainer);

            //顶部标题
            tvTitle = (TextView) findViewById(SobotResourceUtils.getIdByName(context, "id", "tvTitle"));

            //确定和取消按钮
            btnSubmit = (Button) findViewById(SobotResourceUtils.getIdByName(context, "id", "btnSubmit"));
            btnCancel = (ImageView) findViewById(SobotResourceUtils.getIdByName(context, "id", "btnCancel"));
            ...
            //设置文字
            btnSubmit.setText(TextUtils.isEmpty(Str_Submit) ? SobotResourceUtils.getResString(context, "sobot_btn_submit") : Str_Submit);
//            btnCancel.setText(TextUtils.isEmpty(Str_Cancel) ? context.getResources().getString(SobotResourceUtils.getIdByName(context,"string","sobot_btn_cancle")) : Str_Cancel);
...
//            btnCancel.setTextSize(Size_Submit_Cancel);
            ...
            RelativeLayout rv_top_bar = (RelativeLayout) findViewById(SobotResourceUtils.getIdByName(context, "id", "rv_topbar"));
            rv_top_bar.setBackgroundColor(Color_Background_Title == 0 ? pickerview_bg_topbar : Color_Background_Title);
        } else {
            customListener.customLayout(LayoutInflater.from(context).inflate(SobotResourceUtils.getIdByName(context, "layout", layoutRes), contentContainer));
        }
        // 时间转轮 自定义控件
        LinearLayout timePickerView = (LinearLayout) findViewById(SobotResourceUtils.getIdByName(context, "id", "timepicker"));
