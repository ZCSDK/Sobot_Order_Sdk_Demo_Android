...
public class SobotWOCascadeActivity extends SobotWOBaseActivity {
    ...
    private List<SobotCusFieldDataInfoListModel> infoLists = new ArrayList<>();//原始数据，多级
    private List<SobotCusFieldDataInfoListModel> tempInfoLists = new ArrayList<>();//当前这级别
    private LinkedList<SobotCusFieldDataInfoListModel> selectInfoLists = new LinkedList<>();//选中的，从第一级到最后一级（xx省xx市xx县）
    ...
    protected int getContentViewResId() {
        return SobotResourceUtils.getResLayoutId(getSobotBaseContext(), "sobot_activity_wo_cascade");
    }

    @Override
    protected void initView() {
        horizontalScrollView_ll = findViewById(getResId("horizontalScrollView_ll"));
        ...
        if (bundle.getSerializable("cusFieldConfig") != null) {
            cusFieldConfig = (SobotCusFieldConfigModel) bundle.getSerializable("cusFieldConfig");
        }
    ...
    protected void initData() {
        ...
        SobotCusFieldDataInfoListModel cusFieldDataInfoListModel = new SobotCusFieldDataInfoListModel();
    ...
    private void updateIndicator() {
        ...
        for (int i = 0; i < selectInfoLists.size(); i++) {
            View view = LayoutInflater.from(getSobotBaseContext()).inflate(getResLayoutId("sobot_layout_order_category_header_items"), null);
            if (view != null) {
                TextView titleTv = view.findViewById(getResId("work_order_category_title"));
        ...
        if (horizontalScrollView_ll.getChildCount() > 0) {
            for (int i = 0; i < horizontalScrollView_ll.getChildCount(); i++) {
                ...
                TextView titleTv = view.findViewById(getResId("work_order_category_title"));
                View line = view.findViewById(getResId("work_order_category_line"));
