...
import com.sobot.workorder.weight.html.SobotCustomTagHandler;
import com.sobot.workorder.weight.html.SobotLinkMovementClickMethod;
import com.sobot.workorder.weight.html.SobotPhoneSpan;
import com.sobot.workorder.weight.html.SobotURLSpan;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class SobotHtmlTools {

    private static SobotHtmlTools instance;

    public static SobotHtmlTools getInstance(Context context) {
        if (instance == null) {
            instance = new SobotHtmlTools(context.getApplicationContext());
        }
        return instance;
    }
    ...
    private SobotHtmlTools(Context context) {
        super();
    ...
    public void setRichText(TextView widget, String content, int color, boolean showBottomLine) {
        ...
        widget.setMovementMethod(SobotLinkMovementClickMethod.getInstance());
    ...
    public Spanned formatRichTextWithPic(final TextView textView, final String htmlContent, final int color) {
        return Html.fromHtml(htmlContent.replace("span", "sobotspan"), null, new SobotCustomTagHandler(context, textView.getTextColors()));
    }
    ...
    public static void parseLinkText(Context context, TextView widget, Spanned spanhtml, int color, boolean showLine) {
        ...
        if (text instanceof Spannable) {
            ...
            // 检查出所有链接
            Matcher m = getWebUrl().matcher(sp);
            while (m.find()) {
                int start = m.start();
                int e = m.end();
                if (sp.getSpans(start, e, URLSpan.class).length == 0) {
                    sp.setSpan(new SobotURLSpan(context.getApplicationContext(), m.group(), color, true), start, e,
            ...
            while (m.find()) {
                ...
                int e = m.end();
                if (sp.getSpans(start, e, URLSpan.class).length == 0) {
                    sp.setSpan(new SobotPhoneSpan(context.getApplicationContext(), m.group(), color), start, e,
                            Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                }
            }
            ...
            for (URLSpan url : htmlurls) {
                ...
                SobotURLSpan myURLSpan = new SobotURLSpan(context.getApplicationContext(), url.getURL(), color, showLine);
    ...
    public static boolean isHasPatterns(String url) {
        ...
        if (getWebUrl().matcher(url.toString()).matches()) {
            ...
            SobotLogUtils.i("URL 非法，请输入有效的URL链接:" + url);
            return false;
        }
    }
}