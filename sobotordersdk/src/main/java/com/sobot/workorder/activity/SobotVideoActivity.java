...
import com.sobot.workorder.weight.video.StVideoListener;
...
public class SobotVideoActivity extends FragmentActivity implements View.OnClickListener {
    ...
    private AlbumFile mCacheFile;
    private SobotDownloadTask mTask;
    private SobotDownloadListener mDownloadListener;
    private String imageUrL = "";//下载地址
    private Bitmap snapshot ;//缩略图

    /**
     * @param context 应用程序上下文
     * @return
     */
    public static Intent newIntent(Context context, AlbumFile cacheFile) {
        if (cacheFile == null) {
            return null;
        }
        ...
        Bundle b = new Bundle();
        b.putParcelable(EXTRA_VIDEO_FILE_DATA, cacheFile);
        intent.putExtras(b);
        return intent;
    }
    ...
    private void initData() {
        try {
            ...
            mCacheFile = intent.getParcelableExtra(EXTRA_VIDEO_FILE_DATA);
            if (mCacheFile == null ) {
                return;
            }
            if (SobotStringUtils.isNoEmpty(mCacheFile.getFileUrl())) {
                imageUrL = mCacheFile.getFileUrl();
            } else if (SobotStringUtils.isNoEmpty(mCacheFile.getPath())) {
                imageUrL = mCacheFile.getPath();
            }
            SobotDownload.getInstance().setFolder(SobotPathManager.getInstance().getVideoDir());
            if(!imageUrL.startsWith("http")){
                //本地视频，通过uri获取
                showFinishUi(imageUrL);
            }else {
                //远程视频
                restoreTask();
            }
    ...
    private void restoreTask() {
        //更新数据
        SobotProgress progress = SobotDownloadManager.getInstance().get(imageUrL);
        if (progress != null) {
    ...
    private void downloadFile(SobotProgress progress) {
        ...
        mTask = HttpBaseUtils.getInstance().addDownloadFileTask(imageUrL, imageUrL, mCacheFile.getFileName(), null,null);
        if (mTask != null) {
    ...
    private void refreshUI(SobotProgress progress) {
        switch (progress.status) {
            ...
            case SobotProgress.WAITING:
                ...
                if(snapshot!=null) {
                    st_iv_pic.setImageBitmap(snapshot);
                }else {
                    SobotBitmapUtil.display(this, imageUrL, st_iv_pic, 0, 0);
                }
                break;
            ...
            case SobotProgress.FINISH:
                mCacheFile.setPath(progress.filePath);
